  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4">
      ...
      <button
        ...
        onClick={() => {
          if (selectedCellId) {
            mutation.mutate({ cell_id: selectedCellId, load_mw: 280, load_profile: "flat_24_7" });
          }
        }}
      >
        Run Optimizer
      ...
      <div className="mt-3 h-44 rounded-md border border-slate-200 p-2">
        <ResponsiveContainer height="100%" width="100%">
          <LineChart data={result?.pareto_frontier ?? []}>
            ...
            <Tooltip />
            <Line dataKey="effective_cost_eur_mwh" stroke="#176b87" strokeWidth={3} type="monotone" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      {result ? (
