...
import { formatCarbon, formatMw } from "../../lib/formatters";
...
export function RankedSites({ isLoading, results }: RankedSitesProps) {
  ...
  if (isLoading) {
    return <p className="mt-4 text-sm text-slate-500">Loading fixture sites...</p>;
  }
  return (
      ...
      {results.map((result, index) => (
        ...
        >
          ...
          <span className="text-xs text-slate-600">
            Score {result.composite_score} · {formatMw(result.site.headroom_mw)} headroom ·{" "}
